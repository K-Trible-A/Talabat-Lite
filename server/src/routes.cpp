...
void getCustomerData() {
  CROW_ROUTE(server, "/customer/getData/<int>").methods("GET"_method)(
    [](const crow::request &req, int userId) {
      if (userId <= 0) {
        return crow::response(400, "Invalid user ID");
      }
      const string sql1 = "SELECT deliveryAddress FROM customer where userId = " +
...
void setCustomerData() {
  CROW_ROUTE(server, "/customer/setData/<int>").methods("POST"_method)(
  [](const crow::request &req, int userId) {
    if (userId <= 0) {
      return crow::response(400, "Invalid user ID");
    }
    ...
    if (!db.updateData("customer", {"deliveryAddress"}, {deliveryAddress}, condition1)) {
...
void getTopRatedMerchants() {
  CROW_ROUTE(server, "/customer/getTopRatedMerchants/<int>").methods(crow::HTTPMethod::GET)(
    [](const crow::request &req, const int& userId) {
      if (userId <= 0) {
        return crow::response(400, "Invalid user ID");
      }
...
void getCartItems() {
  CROW_ROUTE(server, "/customer/getCartItems/<int>").methods(crow::HTTPMethod::GET)(
      [](const crow::request &req, const int& userId) {
        if (userId <= 0) {
          return crow::response(400, "Invalid user ID");
        }
  ...
  const string sql_all =
      "SELECT i.itemName, ci.quantity, i.itemPrice, i.merchantId ,(ci.Quantity "
  ...
  for (auto &it : ans_all) {
    ...
    const string sql_merch =
        "SELECT businessName FROM merchant WHERE merchantId =" + merchantId +
        " ;";
...
void getCategory() {
   CROW_ROUTE(server, "/customer/getCategory/<int>").methods(crow::HTTPMethod::GET)(
      [](const crow::request &req, const int& businessType) {
    const string sql1 = "SELECT COUNT (*) FROM merchant WHERE businessType = " +
    ...
    const string sql2 = "SELECT userId, businessName , rating FROM merchant "
                        "WHERE businessType = " +
                        std::to_string(businessType) + ";";
    vector<vector<string>> res = db.query(sql2);
    if (res.empty())
...
void addCustomerCard() {
  CROW_ROUTE(server, "/customer/addCard/<int>")
        .methods(crow::HTTPMethod::POST)([](const crow::request &req, const int& userId) {
          auto body = crow::json::load(req.body);
